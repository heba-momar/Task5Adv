      ...
      <Form>
        <div className="d-flex gap-5 align-items-start flex-wrap flex-lg-nowrap">
          ...
          <FormGroup className="mb-3" controlId={`formInput${index}`}>
            ...
            <Form.Control type={input.type} placeholder={input.placeholder} className='rounded-1 w-75'defaultValue={input.value}/>
          </FormGroup>     
                )
             })}
            </div>
          ...
          <div className="flex-fill">
            ...
            <ImageUplood  addNewItem="imageBox rounded-1" onImageSelect= {(file)=>SetselectedImage(file)}/>                  
                    </div>
        </div>
        {/*Button save */}
           <Row>
